...
inline void sort(RandomAccessIterator const first, RandomAccessIterator const middle, RandomAccessIterator const last);

template <class RandomAccessIterator, class Compare>
inline void sort(RandomAccessIterator const first, RandomAccessIterator const middle, RandomAccessIterator const last, Compare comp);
...
template <class RandomAccessIterator, class Compare> class Private
{
    ...
    public:
        class Technique {
            ...
            private:
                ...
                itr_t Copy(itr_t iDst, itr_t iSrc, itr_t eSrc)
                {
                    while (iSrc != eSrc) *iDst++ = std::move(*iSrc++);
                    return iDst;
                }
                
                
                
                itr_t Copy(itr_t iDst, itr_t iSrc, itr_t eSrc, ref_t v)
                {
                    *iDst = std::move(v);
                    return Copy(++iDst, ++iSrc, eSrc);
                }
                        ...
                        if (Comp(v1, v0)){
                            *iJoin++ = std::move(v1);
                            if (--n1){
                                ...
                                iJoin = Copy(iJoin, i0, e0, v0);
                                iJoin = Copy(iJoin, i1+1, e1);
                                break;
                            }
                        } else {
                            *iJoin++ = std::move(v0);
                            if (--n0){
                                ...
                                iJoin = Copy(iJoin, i0+1, e0);
                                iJoin = Copy(iJoin, i1, e1, v1);
                                break;
                            }
                        }
                    }
            ...
            public:
                ...
                Technique(rai_t const first, rai_t const middle, rai_t const last, cmp_t comp)
                :mnOriginal(std::distance(first, last))
                ,maOriginal(first, last)
                ,maExternal(mnOriginal)
                ,mnTop(std::distance(first, middle))
                ,mComp(comp)
                {}
    ...
    public:
        static void Sort(RandomAccessIterator const first, RandomAccessIterator const middle, RandomAccessIterator const last, Compare comp)
        {
            Technique(first, middle, last, comp).Sort();
        }
};
...
inline void sort(RandomAccessIterator const first, RandomAccessIterator const middle, RandomAccessIterator const last)
{
    SakaiShiki::sort(first, middle, last, std::less<typename std::iterator_traits<RandomAccessIterator>::value_type>());
}
...
inline void sort(RandomAccessIterator const first, RandomAccessIterator const middle, RandomAccessIterator const last, Compare comp)
{
    SakaiShiki::Private<RandomAccessIterator, Compare>::Sort(first, middle, last, comp);
}
}
